...
void setup() {
  ...
  // rxPin = GPIO 17 (连接到A7670E的TX)
  // txPin = GPIO 18 (连接到A7670E的RX)
  Serial1.begin(UART_BAUD, SERIAL_8N1, UART_TXD, UART_RXD);
  ...
  Serial.println("  ESP32 GPIO 40 ↔ A7670E RI (可选)");
  Serial.println("  ESP32 GPIO 45 ↔ A7670E DTR (可选)");
  Serial.println("Baud Rate: 115200");
  Serial.println();

  // 硬件连接测试 - 发送测试字节
  Serial.println("Performing hardware connection test...");
  Serial1.write("TEST"); // 发送测试数据
  delay(100);
  if (Serial1.available()) {
    String echo = Serial1.readString();
    if (echo.indexOf("TEST") != -1) {
      Serial.println("⚠️  Warning: Serial echo detected - check if RX/TX are swapped");
    }
  }
  // 清空串口缓冲区
  ...
  if (sendATCommand("AT", 2000)) {
    ...
    Serial.println("   1. Check power: A7670E needs stable 3.3-4.2V power");
    Serial.println("   2. Check wiring:");
    Serial.println("      - ESP32 GPIO 17 ↔ A7670E UART_TXD");
    Serial.println("      - ESP32 GPIO 18 ↔ A7670E UART_RXD");
    Serial.println("      - Ensure GND connection between boards");
    Serial.println("   3. Check baud rate: Should be 115200");
    Serial.println("   4. Try power cycling the A7670E module");
    Serial.println("   5. Check if SIM card is inserted (if required)");
    while(1); // 停止执行，等待用户修复连接
  }
